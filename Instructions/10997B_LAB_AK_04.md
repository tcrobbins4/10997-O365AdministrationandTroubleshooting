...
5. Switch to Microsoft Edge, and then in the Exchange admin center, on **admin** **roles** tab, click  **Refresh**. Ensure that you can see the new  **BranchOffice Admin**s role group.
...
7. Close Windows PowerShell window on LON-CL1.
...
19. Open a new InPrivate window, navigate to  [**https://portal.office.com**](https://portal.office.com) and sign in as **Abbi@Adatum*yyxxxxx*.onmicrosoft.com**, where ***yyxxxxx*** is your unique Adatum number, with Abbi's password. Click  **Mail**.
...
1. On the desktop of  **LON-CL1**, if it is not already opened, right-click  **Windows Azure Active Directory Module for Windows PowerShell**, and then click  **Run as administrator**.
...
9. Switch back to the Microsoft Edge window where you are signed in as  **Ada**. Click the  **Outlook** icon. You should be able to open Outlook on the web now.
