...
10. After Azure AD Connect configuration completes, click  **Exit**.

11. **Sign out** of LON-DS1 and then sign back in as **ADATUM\Administrator**, with the password **Pa55w.rd**. 

>  **NOTE:** Wait for 4-5 minutes for the initial synchronization to finish.

12. Open Internet Explorer on  **LON-DS1**, and then navigate to  [**https://portal.office.com**](https://portal.office.com). Sign in as  **Holly@Adatum*yyxxxxx*.onmicrosoft.com**, where ***yyxxxxx*** is your unique Adatum number, with Holly's password.

13. Click  **Admin**, navigate to  **Users**, and then click  **Active users**.

14. Verify that you can see users from your on-premises AD DS.

15. Minimize the Internet Explorer window that has the Microsoft 365 admin center open.



#### Task 2: Run the Azure AD Connect wizard to modify sync options

1. On the  **LON-DS1** computer, right-click the **Azure AD Connect** icon on the desktop, and then click **Run as administrator**.
...
1. On  **LON-DS1**, click  **Start**, expand the  **Azure AD Connect** folder, right-click **Synchronization Service**, click **More**, and then click **Run as administrator**.
...
13. To close the  **Properties** dialog window, click **OK**. Click **OK** to acknowledge the warning, then close the **Synchronization Service Manager on LON-DS1** window.

14. On  **LON-DS1**, open the  **Start** screen, expand the **Azure AD Connect** folder, right-click **Synchronization Rules Editor**, click **More**, and then click **Run as administrator**.
...
32. On LON-DS1, click **Start**, right-click **Windows PowerShell** , click **More**, and then click **Run as administrator**. 

33. At the Windows PowerShell prompt, type the following command, and then press Enter:

```
Start-ADSyncSyncCycle -PolicyType Initial
```

>  **Note:** This command will manually start synchronization process between AD DS and Azure AD. If you get an error when executing this command, type **Import-Module "C:\\Program Files\\Microsoft Azure AD Sync\\Bin\\ADSync\\ADSync.psd1"**, press **Enter** and then repeat the previous step.
...
1. On  **LON-CL1**, ensure that you are signed in with the administrator account, click **Start**,  type **powershell**, right-click **Windows PowerShell**, and then click **Run as Administrator**. If a User Account Control dialog box appears, click **Yes**.
