...
12. In the  **Microsoft Office 365 admin center**, on the navigation menu to the left, navigate to  **Admin centers**, and then click  **Skype for Business**.

13. On the left-hand side, click  **organization**.

14. On the  **general** page, under **presence privacy mode**, verify that the setting is configured as  **Display presence information only to a user's contacts**.

15. Under  **mobile phone notifications**, verify that  **Apple Push Notification Service** is not enabled, and then click **external communications**.

16. Under  **external access**, verify that  **On except for blocked domains** is selected.

17. Verify that  **under blocked or allowed domains**,  **litware.com** is listed.
